...
6. Click **Run**.
7. Open the Serial Monitor. Gently pinch the thermistor bead between your fingers — the temperature rises and the fan starts spinning. Let go and watch it cool down and stop.

## How it Works

**Flow**

- `setup()` — starts the Serial Monitor, initializes the I2C bus and the motor on M0, and starts with the fan off
...
`(1023.0 / adcValue - 1.0) * seriesResistor` reverses the voltage divider math (the thermistor plus the fixed 10 kΩ resistor, just like the photoresistor in Lesson 4) to recover the thermistor's resistance in ohms — the decimal `1023.0` keeps the division from truncating.
...
Below 25 °C the fan stays off (`power = 0`), above 50 °C it runs at maximum (`power = 100`), and in between `map()` — the same one from Lessons 4 and 5 — spreads the 25–50 °C range across 0–100% so the fan ramps smoothly instead of clicking on.

**Driving the motor**

`motor.setPower()` takes a percentage (−100 to 100) exactly like Lesson 7 — the only new thing here is where the number comes from.
