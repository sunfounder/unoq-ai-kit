...
3. Import `11 IMU Attitude.zip` from `unoq-ai-kit\basic`.
5. Click **Run**.
6. Open the **Serial Monitor**. You'll see accelerometer (m/s²), gyroscope (deg/s), magnetometer (Gauss), azimuth (degrees), temperature (°C), pressure (hPa), and altitude (m) readings every second. Pick up the board and tilt it — watch the values change in real time.

> **Calibration:** for best accuracy, run the **11 IMU Calibration** project first, then copy the calibration output into `calibration_data.h` (in this project's `sketch/` folder) and re-run this sketch with the updated values.
