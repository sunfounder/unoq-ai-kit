...
3. Import `11 IMU Calibration.zip` from `unoq-ai-kit\basic`.
5. Click **Run** and switch to the **Serial Monitor** window.
6. Send any character once to display the first instruction: **Z face up**.
7. Place the device in the requested orientation and keep it completely still.
8. Send any character again to measure the current face.
9. Repeat the process for all six orientations.
10. Copy the printed constants into `calibration_data.h` in the **11 IMU Attitude** project.
...
When all six faces are done, the sketch prints the `ACCEL_BIAS`, `ACCEL_SCALE`, `GYRO_BIAS`, `GYRO_SCALE`, `MAG_BIAS`, and `MAG_SCALE` constants — copy them into `calibration_data.h` in the 11 IMU Attitude project.
