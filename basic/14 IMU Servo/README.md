...
The IMU is connected to the UNO Q QWIIC connector (no wiring needed); connect the two servos to the Robot Shield's servo channels 0 and 1.
...
3. Import `14 IMU Servo.zip` from `unoq-ai-kit\basic`.
5. Connect the battery pack to the Robot Shield.
6. Click **Run**.
7. Hold the board level — both servos center at 0°. Tilt left/right to pan, forward/back to tilt. The servos follow your motion smoothly.

## How it Works

**Flow**

- `setup()` — initializes the Robot Shield and centers both servos at 0°, initializes the IMU, and applies the calibration data from Lesson 11
