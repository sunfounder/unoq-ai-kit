...
socket.on('gesture_action', (message) => {
    if (!message) return;

    currentGesture.textContent = message.gesture || 'Waiting...';

    if (message.action === 'photo') {
        lastAction.textContent = `Photo saved: ${message.photo || ''}`;
    } else if (message.action === 'center') {
        lastAction.textContent = 'Pan-tilt returned to the center';
    } else if (message.action === 'rgb') {
        lastAction.textContent = 'RGB LED updated';
    } else {
        lastAction.textContent = 'Standby';
    }
});
