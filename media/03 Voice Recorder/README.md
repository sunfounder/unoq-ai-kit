...
Connect the two push buttons between D2 / D3 and GND:
...
5. Wait for `Ready.` in the **Output** window, then:
   - Press the **D2 button** once — **Recording...** appears; speak into the microphone.
   - Press **D2** again — **Recording saved.** appears.
   - Press the **D3 button** once — **Playing...** appears and the speaker plays your recording.
   - Press **D3** again to stop playback.
...
- D2 pressed → `record_button_read` returns 1 → `stt.start_recording()`
- D2 pressed again → `stt.stop_recording(AUDIO_FILE)` → recording saved to `stt_last.wav`
- D3 pressed → `tts.play_audio(AUDIO_FILE)` → speaker plays the file
- D3 pressed again → `tts.stop_audio()` → playback stops
